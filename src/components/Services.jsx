        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {services.map((svc, idx) => {
            ...
            return (
              <div 
                key={idx}
                className="group relative p-8 bg-charcoal-dark/30 border border-white/5 hover:border-gold/20 transition-all duration-500 flex flex-col justify-between min-h-[220px]"
              >
                {/* Structural Line Overlays */}
                <div className="absolute top-0 left-0 w-8 h-[1px] bg-gold/10 group-hover:bg-gold/40 transition-colors duration-500" />
                <div className="absolute top-0 left-0 w-[1px] h-8 bg-gold/10 group-hover:bg-gold/40 transition-colors duration-500" />

                <div className="space-y-6">
                  {/* Icon & Title */}
                  <div className="flex items-center gap-4">
                    <div className="p-3 bg-charcoal-deep border border-white/10 text-gold group-hover:text-white group-hover:bg-gold transition-colors duration-500">
                      <Icon className="h-5 w-5" />
                    </div>
                    <h3 className="font-display text-lg text-white font-light group-hover:text-gold transition-colors duration-300">
                      {svc.title}
                    </h3>
                  </div>
                ...
                {/* Numbering Indicator */}
                <span className="self-end font-display text-[10px] tracking-widest text-slate-700 group-hover:text-gold/40 transition-colors duration-500 pt-6">
